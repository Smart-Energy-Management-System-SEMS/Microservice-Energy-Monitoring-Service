# Application layer: this service only READS readings (it never changes data).
...
class EnergyReadingQueryService:
    """Different ways to look for readings in the database."""
    ...
    async def get_by_user(self, query: GetByUserQuery) -> List[EnergyReading]:
        """All readings that belong to one user."""
    ...
    async def get_by_id(self, reading_id: str) -> Optional[EnergyReading]:
        """One reading by its id (or None if it does not exist)."""
    ...
    async def get_latest_by_meter(self, meter_id: str) -> Optional[EnergyReading]:
        """The most recent reading of a meter."""
