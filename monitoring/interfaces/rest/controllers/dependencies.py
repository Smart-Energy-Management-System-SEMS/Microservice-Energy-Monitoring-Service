...
from monitoring.infrastructure.external.eos.mock_eos_iot_adapter import MockEosIotAdapter
from monitoring.infrastructure.external.pricing.mock_plus_energia_adapter import MockPlusEnergiaAdapter
...
def get_eos_provider():
    return MockEosIotAdapter()


def get_pricing_provider():
    return MockPlusEnergiaAdapter()
...
def get_energy_simulation_command_service():
    return EnergySimulationCommandService(
        get_eos_provider(),
        get_pricing_provider(),
        get_reading_command_service(),
        get_event_publisher(),
    )
...
def get_energy_simulation_query_service():
    return EnergySimulationQueryService(get_reading_repo(), get_pricing_provider())
